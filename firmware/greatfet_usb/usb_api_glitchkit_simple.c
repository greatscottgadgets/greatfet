...
static void glitchkit_gpio_isr(void);
...
static uint32_t event_count_target = 0;
...
bool active_condition_level[MAX_PIN_CONDITIONS];
struct gpio_t active_condition_gpio[MAX_PIN_CONDITIONS];
uint8_t active_condition_count = 0;
...
uint8_t active_interrupt_mask = 0;
 ...
 *    setup value: high byte of the 32-bit counter
 ...
 */
static void handle_glitchkit_event(void) {
	++event_count;
