 ...
 * K.Temkin 2018-12-07 - Move to the libgreatfet codebase, for use as our allocator.
 * ----------------------------------------------------------------------------
 */
...
static void umm_assimilate_up( unsigned short int c ) {
  ...
  if( UMM_NBLOCK(UMM_NBLOCK(c)) & UMM_FREELIST_MASK ) {
    ...
    pr_debug( "Assimilate up to next block, which is FREE\n" );
...
void umm_free( void *ptr ) {
  ...
  if( (void *)0 == ptr ) {
    pr_debug( "free a null pointer -> do nothing\n" );
  ...
  if( UMM_NBLOCK(UMM_PBLOCK(c)) & UMM_FREELIST_MASK ) {
    ...
    pr_debug( "Assimilate down to next block, which is FREE\n" );
    ...
    pr_debug( "Just add to head of free list\n" );
...
void *umm_malloc( size_t size ) {
  ...
  if( 0 == size ) {
    pr_debug( "malloc a block of 0 bytes -> do nothing\n" );
  ...
  while( cf ) {
    ...
    pr_trace( "Looking at block %6i size %6i\n", cf, blockSize );
...
#  error "No UMM_*_FIT is defined - check umm_malloc_cfg.h"
  ...
  if( UMM_NBLOCK(cf) & UMM_BLOCKNO_MASK && blockSize >= blocks ) {
    ...
    if( blockSize == blocks ) {
      ...
      pr_debug( "Allocating %6i blocks starting at %6i - exact\n", blocks, cf );
      ...
      pr_debug( "Allocating %6i blocks starting at %6i - existing\n", blocks, cf );
    ...
    pr_debug(  "Can't allocate %5i blocks\n", blocks );
...
void *umm_realloc( void *ptr, size_t size ) {
  ...
  if( ((void *)NULL == ptr) ) {
    pr_debug( "realloc the NULL pointer - call malloc()\n" );
  ...
  if( 0 == size ) {
    pr_debug( "realloc to 0 size, just free the block\n" );
  ...
  pr_debug( "realloc blocks %i blockSize %i nextBlockSize %i prevBlockSize %i\n", blocks, blockSize, nextBlockSize, prevBlockSize );
   ...
   */
    ...
    if (blockSize >= blocks) {
        pr_debug( "realloc the same or smaller size block - %i, do nothing\n", blocks );
        /* This space intentionally left blank */
    } else if ((blockSize + nextBlockSize) >= blocks) {
        pr_debug( "realloc using next block - %i\n", blocks );
        umm_assimilate_up( c );
        blockSize += nextBlockSize;
    } else if ((prevBlockSize + blockSize) >= blocks) {
        pr_debug( "realloc using prev block - %i\n", blocks );
        ...
        pr_debug( "realloc using prev and next block - %i\n", blocks );
        umm_assimilate_up( c );
        umm_disconnect_from_free_list( UMM_PBLOCK(c) );
        c = umm_assimilate_down(c, 0);
        memmove( (void *)&UMM_DATA(c), ptr, curSize );
        ...
        pr_debug( "realloc a completely new block %i\n", blocks );
        void *oldptr = ptr;
        if( (ptr = umm_malloc( size )) ) {
            pr_debug( "realloc %i to a bigger block %i, copy, and free the old\n", blockSize, blocks );
            memcpy( ptr, oldptr, curSize );
            umm_free( oldptr );
        } else {
            pr_debug( "realloc %i to a bigger block %i failed - return NULL and leave the old block!\n", blockSize, blocks );
    ...
    if (blockSize > blocks ) {
        pr_debug( "split and free %i blocks from %i\n", blocks, blockSize );
